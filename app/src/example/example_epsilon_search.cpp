...
int main()
{
    ...
    for (int i = 0; i < num_queries; i++) {
        ...
        std::vector<std::pair<float, hnswlib::labeltype>> result_hnsw = alg_hnsw->searchStopConditionClosest(query_data, stop_condition);
        ...
        std::priority_queue<std::pair<float, hnswlib::labeltype>> result_brute = alg_brute->searchKnn(query_data, max_elements);
    ...
    for (size_t i = 0; i < max_elements; i++) {
        ...
        std::vector<std::pair<float, hnswlib::labeltype>> result = alg_hnsw->searchStopConditionClosest(alg_hnsw->getDataByInternalId(i), stop_condition);
