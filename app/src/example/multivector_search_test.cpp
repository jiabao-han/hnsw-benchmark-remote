...
int main()
{
    ...
    for (int i = 0; i < num_queries; i++) {
        ...
        std::vector<std::pair<dist_t, hnswlib::labeltype>> hnsw_results = alg_hnsw->searchStopConditionClosest(query_data, stop_condition);
        ...
        std::vector<std::pair<dist_t, hnswlib::labeltype>> gt_results = alg_brute->searchKnnCloserFirst(query_data, max_elements);
    ...
    for (int i = 0; i < max_elements; i++) {
        hnswlib::MultiVectorSearchStopCondition<docidtype, dist_t> stop_condition(space, num_docs, ef_collection);
        std::vector<std::pair<float, hnswlib::labeltype>> result = alg_hnsw->searchStopConditionClosest(data + i * data_point_size, stop_condition);
