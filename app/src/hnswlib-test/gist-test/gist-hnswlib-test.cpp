    ...
    for (int M : M_VALUES) {
        for (int ef_construction : EF_CONSTRUCTION_VALUES) {
            ...
            try {
                ...
                hnswlib::L2Space space(GIST_DIM);
...
int main()
{
    try {
        ...
        std::string index_base_path = std::string(index_dir) + "hnswlib-test/gist-test/gist-test";
        std::string result_file_path = std::string(result_dir) + "hnswlib-test/gist/gist_recall_qps_result.csv";
