...
#include "AngularSpace.h"

const size_t GLOVE_200_DIM = 200;
    ...
    for (int M : M_VALUES) {
        for (int ef_construction : EF_CONSTRUCTION_VALUES) {
            ...
            try {
                ...
                hnswlib::AngularSpace space(GLOVE_200_DIM);
...
int main()
{
    try {
        ...
        std::string index_base_path = std::string(index_dir) + "hnswlib-test/glove-200-angular-test/glove-200-angular-test";
        std::string result_file_path = std::string(result_dir) + "hnswlib-test/glove-200-angular/glove-200-angular_recall_qps_result.csv";
