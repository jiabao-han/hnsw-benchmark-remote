...
const size_t SIFTSMALL_DIM = 128;
const size_t K = 10; // Number of nearest neighbors to retrieve
const size_t NUM_FOLDS = 5; // Number of folds for cross-validation
    ...
    for (int M : M_VALUES) {
        for (int ef_construction : EF_CONSTRUCTION_VALUES) {
            ...
            try {
                ...
                hnswlib::L2Space space(SIFTSMALL_DIM);
...
int main()
{
    try {
        ...
        std::string index_base_path = std::string(index_dir) + "hnswlib-test/siftsmall-test/siftsmall-test";
        std::string result_file_path = std::string(result_dir) + "hnswlib-test/siftsmall/siftsmall_recall_qps_result.csv";
